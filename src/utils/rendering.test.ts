...
describe('getRowColorClass', () => {
  it('returns title class for row 0 with content', () => {
    expect(getRowColorClass(0, 'MAIN MENU')).toBe('gs-row-title');
  });

  it('returns content class for row 0 with empty content', () => {
    expect(getRowColorClass(0, '   ')).toBe('gs-row-content');
  });

  it('returns fkey class for rows 22-23', () => {
    expect(getRowColorClass(22, 'F3=Exit  F12=Cancel')).toBe('gs-row-fkey');
    expect(getRowColorClass(23, 'F3=Exit')).toBe('gs-row-fkey');
  });

  it('returns fkey class for rows with F-key references', () => {
    expect(getRowColorClass(15, 'Press F3=Exit to leave')).toBe('gs-row-fkey');
  });

  it('returns subtitle class for "Select one of the following"', () => {
    expect(getRowColorClass(5, '  Select one of the following')).toBe('gs-row-subtitle');
  });

  it('returns content class for regular rows', () => {
    expect(getRowColorClass(10, 'Some regular content here')).toBe('gs-row-content');
  });
});
...
describe('parseHeaderRow', () => {
  ...
  it('parses header with left and right system names', () => {
    ...
    expect(result[0].colorClass).toBe('gs-row-fkey');
    expect(result[1].colorClass).toBe('gs-row-title');
    expect(result[2].colorClass).toBe('gs-row-content');
  });

  it('returns single title segment when only one word', () => {
    const result = parseHeaderRow('TITLE')!;
    expect(result).toHaveLength(1);
    expect(result[0].colorClass).toBe('gs-row-title');
  });
});
